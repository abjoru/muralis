...
**Ultrawide Source**:
The **Browsed Source** for ultrawidewallpapers.net (`muralis-source-ultrawide`), and the first Source to publish **Categories**. Emphatically *not* a **RestSource**: the site has no REST/JSON API, no feed and no text search, so there is no paged JSON for a **Source Descriptor** to drive and no query dimension to expose. Its nearest sibling is the **Feed Source** — non-REST, parsing markup, browsed. One `[sources.ultrawide]` subsection, `enabled = false` by default, with an optional `tags` list replacing the **Shipped tag vocabulary**. Its display name — what `browse` and the GUI chip name it by — is the single word `Ultrawide`, fixed in the crate like every other built-in: one word so no invocation needs shell quoting, and cosmetic, never the **Library** key (that is `source_type`). Multi-word names belong to user-configured instances (**Feed Source** entries, **Booru Source** hosts), where the name is the user's own to quote.
_Avoid_: scraper source, ultrawide plugin (reserve "plugin" for the crate), UWW, any two-word display name (the pre-release spelling; it never shipped)

**Gallery endpoint**:
`gallery_load.php?offset=<n>&limit=<n>[&tag=A,B]` — the whole of the **Ultrawide Source**'s retrieval, and the site's own infinite scroll. Takes a zero-based offset and a limit, answers with an HTML fragment of bare **Cards**, and intersects a comma-separated tag list. A logical page asks for exactly the window it will return, so a page is *fetched once* and nothing larger is fetched to be sliced. `limit` is uncapped upstream and that stays out of bounds: a request asks for the window being displayed and no more. An *empty fragment* is the end of the results — for an offset past the end and for a tag the site does not know alike — and becomes the empty result muralis already treats as the end. A fragment that arrived with content and yielded no **Card** is the opposite: a markup change, reported loudly.
_Avoid_: API (there is no JSON and no contract), search endpoint (it has no query dimension)

**Shipped tag vocabulary**:
The 28 tags the gallery page publishes as `data-tag` attributes, shipped whole in the crate, in the site's own casing. A tag is its own slug *and* its own label — the site publishes names a person reads, so there is nothing to derive. Unlike the ~88 category slugs this replaced, every one of these narrows something real, so shipping all of them is a usable menu rather than a taste call; config names a subset when a shorter one is wanted. The site's navigation is never scraped at runtime to discover them, and the **Drift check** asks weekly whether the list still matches.
_Avoid_: shipped categories (the retired eight slugs), tag cloud

**Category page** (retired):
The curated one-URL-per-slug listings the Source used to browse, and the model it was built on: an editorial slice with no pagination, so a logical `page` was a `skip`/`take` over one parsed page that was *refetched* per logical page, and whose cards were three-quarters lazy-loaded behind a `data-src` placeholder. It lost on every axis that mattered — coverage (8 hand-picked slugs out of 78, most resolution-flavoured and filtering nothing, since every **Master** is 7680x2160 regardless), paging, thumbnails, and combination (one editorial slice cannot be asked for two things at once). Tags replaced it rather than joining it: two browse paths in one Source is two parsers to keep alive and two ways to do the same thing. The category-page parser, the shipped slug list and the slice-a-larger-set paging are gone from the tree; a config still carrying `categories` is warned about by name rather than read as naming nothing.

**Card**:
One entry in the **Gallery endpoint**'s fragment: an anchor to the full-resolution **Master** carrying its filename, wrapping the thumbnail `<img>`. Parsing yields one **Preview** per distinct filename. The endpoint writes the real thumbnail URL into a plain `src` and leaves the deferral to the browser's own `loading="lazy"`, so the 1x1 placeholder the retired **Category page** needed a second attribute for is not part of this contract — but the value is still checked rather than trusted. A `data:` URI never reaches a Preview: it is a value, not a location, and a placeholder is a valid image that loads successfully and paints nothing, so a Preview carrying one fails *silently* — the grid renders blank and no error handling built around load failure ever sees it. A card with no fetchable thumbnail URL is therefore drift like any other markup change, reported loudly for the whole window and naming the request, rather than emitted as an unusable Preview.

**Category bar**:
The GUI's presentation of a **Browsed Source**'s **Categories**, below the filter bar, by display label, and the surface a **Category selection** is built on. Shown only for a Browsed Source that publishes at least one — **Zero categories** shows no bar, because selecting the feed is already the selection. What a pick does is read from the Source's **Combinable categories** declaration, never assumed: where they combine the chips *toggle* and the selection is a set; where they do not, a pick *replaces*, one at a time as it always was. Selecting is what retrieves — it issues the **Browse verb** for the selection's first page — but the retrieval waits out a **Settling delay** first, so a handful of chips picked in succession is one request for the final selection. Changing the selection resets to page 1; paging pages inside it. A Browsed Source is offered no search field and no aspect combo at all, since it has no query dimension to answer them with.
_Avoid_: category filter (it selects what is retrieved, it does not narrow a result set), tab bar

**Settling delay**:
The pause the **Category bar** leaves between a selection changing and the retrieval it asks for. The selection itself moves at once — the chips must follow the click — and only the request waits, so the last selection is what is reflected rather than each step towards it. Keyboard *toggling* does not settle at all: it is deliberately distinguishable from *committing*, so a multi-chip selection can be built without a request per keystroke.
_Avoid_: debounce (that is the search field's throttle on typing, a different thing on a different dimension)

**Master gate**:
The site hotlink-protects its **Master** paths at the origin: a request that does not say which page it came from is answered `403`, with the rate limiter's headroom untouched. Only Cloudflare masks it — a cache HIT never reaches the origin, so a popular Master serves to anyone and a cold one refuses everyone, which is why the defect read as "some files are forbidden" rather than as one rule. Master requests therefore carry a `Referer` naming the gallery page on *the same host the Preview's URLs name* — the header a browser sends when the gallery link is clicked. This Source exists to render that gallery, so the header is the request the site expects from the page it serves; it is part of the **Access posture**, not a way around a control. It is local to this Source and to the GUI's fetch of the same protected paths: no other **Source** sends one, and a 403 (a gate) is never reported as a 429 (our own volume), in the Source's errors or in the **Drift check**'s.
_Avoid_: hotlink workaround, referer spoofing (the page named is the page being rendered)
...
**Access posture**:
Part of the **Ultrawide Source**'s contract, not an optimisation: one request per user action — and it asks for exactly the window being displayed, not a 200 KB page refetched once per logical page — thumbnails served from the local thumbnail cache rather than re-fetched per render (the Source itself never fetches a thumbnail; it passes the card's URL on), a muralis-identifying `User-Agent` on every outbound request, a `Referer` naming the gallery on the **Master** requests the **Master gate** demands one for and on nothing else, attribution plus a `source_url` link-back to the gallery on every **Preview**, and no enumeration beyond what one window returns. The site's terms restrict automated tools; this integration is a user-initiated renderer producing a browser's request volume, and these constraints are what keep that true as the feature evolves.

**Drift check**:
The weekly scheduled job that asks whether ultrawidewallpapers.net still yields usable **Previews** (`.github/workflows/ultrawide-drift.yml`, driven by `muralis-source-ultrawide::check_live_gallery`). The **Card** fixtures are frozen on their capture date, so the offline suite passes however far the site moves — confidence grows exactly as accuracy decays. The check fetches one window of the **Gallery endpoint** and runs the *production* parser over it, never a reimplementation: a check with its own parsing logic can only disagree with the real one. "Did anything parse?" is not the question, because the Source shipped reading `src` on a page that lazy-loads and produced a full complement of Cards every one of which carried a 1x1 placeholder. So the check asks whether what parsed is *usable*, in named questions, cheapest first: `cards-present` (the fragment had Cards at all — an *empty* window for a tag the site publishes is drift here, though the same emptiness is the end-of-results signal in production), `fields-usable` (no thumbnail or master URL that is empty, a `data:` URI, relative, or the request URL itself), `card-count` (a plausible band within the window it asked for — a window asked for 24 and answered with three is a parser reading something that is no longer the card list), `tag-vocabulary`, `urls-resolve` (both URLs of a small sample — first Card and last — fetched a byte at a time, *each as the Source itself fetches it*, so the Master carries the `Referer` the **Master gate** demands and a gate failing is caught here rather than by a user trying to keep one) and `urls-refused` (a sampled URL the site has and will not serve to that request: a 403, which means the gate has moved, and is a different job from a URL the site does not have and from being asked to slow down). A report names which question failed and shows the offending value, so "no cards", "cards without usable fields", "a stale menu", "fields that do not resolve" and "a field the site refuses" never blur together.

`tag-vocabulary` is the question the markup cannot answer: it fetches the gallery page, reads the tags it publishes as `data-tag` attributes, and compares them as a *set* with the **Shipped tag vocabulary**, naming what was added and what was removed. It exists because the **Gallery endpoint** answers a tag it does not know with the same empty fragment it answers a real tag with nothing in it — so a shipped tag the site has dropped reads to a user as "nothing found", and would otherwise be found by report rather than on a schedule.

The check is emphatically not part of the suite, which stays offline and deterministic; it never runs on push or pull request, and its one window, one gallery page and four one-byte verifications per week carry the **muralis User-Agent**, consistent with the **Access posture**. Only drift is filed as an issue — a timeout, a 5xx and a rate limit, on either page or on a verification request, are transient and say nothing about the markup or the vocabulary. Success is silent, and refreshing a fixture stays a human decision, because a new capture is a new parser contract.
...
- Every muralis entry point ensures its own XDG directories before touching them — the daemon at startup, the CLI in every command that opens the **Library** or writes a file. The CLI cannot inherit the daemon's startup for this: the **CLI contract** promises the Library reads with the daemon down, and keeping is a CLI-side operation, so a CLI-first machine has directories only if the CLI makes them. Creation is idempotent and confined to muralis's own locations, so the two starting at once is a race neither loses. An IO failure against one of those locations names the operation and the path (`MuralisError::IoAt`); a bare errno names neither and has been mistaken for a fault in whatever command tripped it.
