...
A card comes in two forms, and the minority form is the eager one: three quarters of a category's cards are **lazy**, their `src` holding a 1x1 transparent GIF data URI while the real thumbnail URL waits in `data-src` for the site's own script to promote it on scroll. The parser reads whichever attribute holds a fetchable URL, so the two forms yield identical Previews; a card's position on the page decides nothing. A `data:` URI never reaches a Preview — it is a value, not a location, and a placeholder is a valid image that loads successfully and paints nothing, so a Preview carrying one fails *silently*: the grid renders blank and no error handling built around load failure ever sees it. A card with no fetchable thumbnail URL in any attribute is therefore drift like any other markup change, reported loudly for the whole page rather than emitted as an unusable Preview.
...
**Drift check**:
The weekly scheduled job that asks whether ultrawidewallpapers.net still yields usable **Previews** (`.github/workflows/ultrawide-drift.yml`, driven by `muralis-source-ultrawide::check_live_category`). The **Card** fixtures are frozen on their capture date, so the offline suite passes however far the site moves — confidence grows exactly as accuracy decays. The check fetches one **Category page** and runs the *production* parser over it, never a reimplementation: a check with its own parsing logic can only disagree with the real one. "Did anything parse?" is not the question, because the Source shipped reading `src` on a page that lazy-loads and produced a full complement of Cards every one of which carried a 1x1 placeholder. So the check asks whether what parsed is *usable*, in four named questions, cheapest first: `cards-present` (the page had Cards at all), `fields-usable` (no thumbnail or master URL that is empty, a `data:` URI, relative, or the Category page itself), `card-count` (a plausible band — fewer than one carousel page of twelve is a parser reading something that is no longer the card list), and `urls-resolve` (both URLs of a small sample — first Card and last, so both the eager and the lazy form are covered — fetched a byte at a time to confirm the site serves them). A report names which question failed and shows the offending value, so "no cards", "cards without usable fields" and "fields that do not resolve" never blur together. It is emphatically not part of the suite, which stays offline and deterministic; it never runs on push or pull request, and its one page fetch plus four one-byte verifications per week carry the **muralis User-Agent**, consistent with the **Access posture**. Only drift is filed as an issue — a timeout, a 5xx and a rate limit, on the page or on a verification request, are transient and say nothing about the markup. Success is silent, and refreshing a fixture stays a human decision, because a new capture is a new parser contract.
